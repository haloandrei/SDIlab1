...
public class DataBase {
    ...
    private static List<Client> getClientsWithMoneyGreater(double money) throws SQLException {
        List<com.haloandrei.domain.Client> result = new ArrayList<>();
        ...
        while (resultSet.next()) {
            ...
            String name = resultSet.getString("name");
            double moneySpent = resultSet.getDouble("moneyspent");
            com.haloandrei.domain.Client client = new com.haloandrei.domain.Client(name, moneySpent);
            client.setId(id);
            result.add(client);
        }
        return result;
